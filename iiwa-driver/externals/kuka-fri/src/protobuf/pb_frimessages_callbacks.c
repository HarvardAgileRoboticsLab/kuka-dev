...
bool encode_repeatedTransformation(pb_ostream_t *stream, const pb_field_t *field, void * const *arg)
{
   int i = 0;
   tRepeatedTransformationArguments* arguments = 0;
   int count = 0;
   ...
   arguments = (tRepeatedTransformationArguments*) *arg;
   ...
   for (i = 0; i < count; i++)
   {
      ...
      if (!pb_encode_submessage(stream, Transformation_fields, &values[i]))
      {
         return false;
      }
   }
...
bool decode_repeatedTransformation(pb_istream_t *stream, const pb_field_t *field, void **arg)
{
   tRepeatedTransformationArguments* arguments = 0;
   size_t i = 0;
   Transformation* values = 0;
   ...
   if (!pb_decode(stream, Transformation_fields, &values[i]))
   {
      return false;
   }
...
void map_repeatedTransformation(eNanopbCallbackDirection dir, int numberOfElements, pb_callback_t *values,
      tRepeatedTransformationArguments *arg)
{
   // IMPORTANT: the callbacks are stored in a union, therefor a message object
   // must be exclusive defined for transmission or reception
   if (dir == FRI_MANAGER_NANOPB_ENCODE)
   {
      // set the encode callback function
      values->funcs.encode = &encode_repeatedTransformation;
   }
   else
   {
      // set the decode callback function
      values->funcs.decode = &decode_repeatedTransformation;
   }
   // map the robot drive state from the container to message field
   arg->max_size = numberOfElements;
   arg->size = 0;
   if (numberOfElements)
   {
      arg->value = (Transformation*) malloc(numberOfElements * sizeof(Transformation));
      memset(arg->value, 0, numberOfElements * sizeof(Transformation));
   }
   values->arg = arg;
}

void init_repeatedTransformation(tRepeatedTransformationArguments *arg)
{
   arg->size = 0;
   arg->max_size = 0;
   arg->value = NULL;
...
void free_repeatedTransformation(tRepeatedTransformationArguments *arg)
{
   if (arg->value != NULL)
   {
      free(arg->value);
